...
export function DiscoveryGrid({ items, loading, hasMore, onLoadMore, onFavoriteChange }) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-4">
      ...
      <div className="flex gap-2.5 sm:gap-5 md:gap-6 w-full items-start" style={{ alignItems: 'flex-start' }}>
        {columns.map((colItems, colIdx) => (
          <div key={colIdx} className="flex flex-col gap-2.5 sm:gap-5 md:gap-6 flex-1 min-w-0">
            {colItems.map((item) => (
              <DiscoveryCard
                key={item.id}
                item={item}
