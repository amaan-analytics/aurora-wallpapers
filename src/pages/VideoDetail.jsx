...
export function VideoDetail() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        if (data) {
          // Load similar videos
          const similar = await getVideos(data.title || 'creative', 1, 8);
          setSimilarVideos(similar.items?.filter(v => v.id !== data.id) || []);
  ...
  const handleFavoriteToggle = async () => {
    ...
    try {
      if (isFavorited) {
        await removeFavorite(currentUserId, video.id);
        setIsFavorited(false);
      } else {
        await addFavorite(currentUserId, video);
        setIsFavorited(true);
      }
    } catch (err) {
  ...
  const handleDownload = async () => {
    ...
    try {
      await addDownload(user?.uid || null, video);
      ...
      a.href = blobUrl;
      a.download = `aurora-video-${video.id}.mp4`;
  ...
  const shareVideo = async () => {
    if (!video) return;
    const shareData = {
      title: `${video.title} | Aurora Videos`,
      text: `Check out this stock footage on Aurora!`,
  ...
  if (!video) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <button onClick={() => navigate('/videos')} className="text-accent-theme flex items-center gap-2">
  ...
  const sourceName = video.id.includes('pexels') ? 'Pexels' : video.id.includes('pixabay') ? 'Pixabay' : 'Aurora Content';

  return (
    <div className="max-w-7xl mx-auto px-4 pt-6 pb-24 sm:py-8 space-y-12 lg:pb-8">
      <SEO 
        title={`${video.title} | Aurora Videos`}
        description={`Download high resolution video footage by ${video.photographer}.`}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col items-center justify-center relative bg-black border border-border-theme/30 rounded-3xl overflow-hidden min-h-[40vh] lg:min-h-[60vh]">
          <video
            src={video.video_url}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-background-theme/60 p-3 rounded-xl border border-border-theme/20">
                ...
                <span className="text-xs font-semibold text-text-primary uppercase flex items-center gap-1.5">
                  <Video className="w-3.5 h-3.5 text-accent-theme" />
                  MP4 Video
                </span>
              </div>
            </div>
